   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private function onConfirmDialogEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            ...
            case "confirm_buy_button":
               var_70.buyMarketPlaceOffer(var_551);
               if(var_139)
               {
                  var_139.dispose();
               }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            if(parser.offerId > 0)
            {
               newItem = var_230.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
               var_230.remove(parser.requestedOfferId);
               newItem.offerId = parser.offerId;
               newItem.price = parser.newPrice;
               var_230.add(parser.offerId,newItem);
            }
            else
            {
               var_230.remove(parser.requestedOfferId);
         ...
         {
            var_230.remove(parser.offerId);
            var_551 = -1;
            _visualization.listUpdatedNotify();
         }
         else if(parser.result == 3)
         {
            updateItem = var_230.getValue(var_551) as MarketPlaceOfferData;
            var_230.remove(parser.requestedOfferId);
            updateItem.offerId = parser.offerId;
            updateItem.price = parser.newPrice;
            var_230.add(parser.offerId,updateItem);
            showConfirmation(const_1512);
            _visualization.listUpdatedNotify();
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_663.add(_loc4_.offerId,_loc5_);
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_663.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_663.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_428)
            {
               var_663.remove(_loc2_);
            }
         }
      ...
      private function showConfirmation(param1:int) : void
      {
         ...
         switch(param1)
         {
            case const_1454:
               ...
               var_139 = _windowManager.buildFromXML(_loc2_) as IWindowContainer;
               break;
            case const_1512:
         ...
         if(var_139)
         {
            var_139.procedure = onConfirmDialogEvent;
            var_139.visible = true;
            var_139.center();
            _loc3_ = var_230.getValue(var_551) as MarketPlaceOfferData;
            if(!_loc3_)
            {
               return;
            }
            _loc4_ = var_139.findChildByName("item_name") as ITextWindow;
            if(_loc4_ != null)
            {
               _loc4_.text = "${roomItem.name." + _loc3_.furniId + "}";
            }
            _loc5_ = var_139.findChildByName("item_desc") as ITextWindow;
            if(_loc5_ != null)
            {
               _loc5_.text = "${roomItem.desc." + _loc3_.furniId + "}";
            }
            _loc6_ = var_139.findChildByName("item_price") as ITextWindow;
            if(_loc6_ != null)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_230.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
