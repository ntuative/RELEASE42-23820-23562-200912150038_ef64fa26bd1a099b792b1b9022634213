package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1427:String = "_INCLUDE";
      
      public static const const_980:String = "_EXCLUDE";
      
      public static const const_1411:String = "_TEMP";
      
      private static const const_1074:RegExp = /^(\s|\n|\r|\t|\v)*/m;
      
      private static const const_1073:RegExp = /(\s|\n|\r|\t|\v)*$/;
       
      
      protected var var_1046:Dictionary;
      
      private var _disposed:Boolean = false;
      
      protected var var_877:Dictionary;
      
      protected var var_1300:Map;
      
      protected var _context:IWindowContext;
      
      protected var var_1299:Dictionary;
      
      protected var var_1047:Dictionary;
      
      public function WindowParser(param1:IWindowContext)
      {
         super();
         _context = param1;
         var_1047 = new Dictionary();
         var_1046 = new Dictionary();
         TypeCodeTable.fillTables(var_1047,var_1046);
         var_877 = new Dictionary();
         var_1299 = new Dictionary();
         ParamCodeTable.fillTables(var_877,var_1299);
         var_1300 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1073,"");
         return param1.replace(const_1074,"");
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_1047)
            {
               delete var_1047[_loc1_];
            }
            for(_loc1_ in var_1046)
            {
               delete var_1046[_loc1_];
            }
            for(_loc1_ in var_877)
            {
               delete var_877[_loc1_];
            }
            for(_loc1_ in var_1299)
            {
               delete var_1299[_loc1_];
            }
            var_1300.dispose();
            var_1300 = null;
            _context = null;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         background = parseAttribute(xml,"background",variables,background.toString()) == "HabboAvatarRenderLib_fx15";
         treshold = uint(parseAttribute(xml,"treshold",variables,treshold.toString()));
         visible = parseAttribute(xml,"visible",variables,visible.toString()) == "HabboAvatarRenderLib_fx15";
         clipping = parseAttribute(xml,"clipping",variables,clipping.toString()) == "HabboAvatarRenderLib_fx15";
         ...
         if(window != null)
         {
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  ...
                  {
                     if((params & 0) == WindowParam.const_157)
                     {
                        window.x = rect.x;
                     }
                     if((params & 0) == WindowParam.const_151)
                     {
                        window.y = rect.y;
                     }
                  }
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_980) == -1)
                  {
