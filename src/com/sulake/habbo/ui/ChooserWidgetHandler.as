package com.sulake.habbo.ui
{
   ...
   public class ChooserWidgetHandler implements IRoomWidgetHandler
   {
      ...
      private function handleUserChooserRequest() : void
      {
         ...
         var _loc3_:* = [];
         var _loc5_:int = _container.roomEngine.getRoomObjectCount(_loc1_,_loc2_,RoomObjectCategoryEnum.const_35);
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
            _loc4_ = _container.roomEngine.getRoomObjectWithIndex(_loc1_,_loc2_,_loc6_,RoomObjectCategoryEnum.const_35);
            _loc7_ = _container.roomSession.userDataManager.getUserDataByIndex(_loc4_.getId());
            if(_loc7_ != null)
            {
               _loc3_.push(new ChooserItem(_loc7_.id,RoomObjectCategoryEnum.const_35,_loc7_.name));
            }
            _loc6_++;
         }
         _loc3_.sort(compareItems);
         _container.events.dispatchEvent(new RoomWidgetChooserContentEvent(RoomWidgetChooserContentEvent.const_392,_loc3_));
      }
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRequestWidgetMessage.const_309);
         _loc1_.push(RoomWidgetRequestWidgetMessage.const_389);
         _loc1_.push(RoomWidgetRoomObjectMessage.const_670);
         return _loc1_;
      }
      ...
      private function handleFurniChooserRequest() : void
      {
         ...
         var _loc2_:int = _container.roomSession.roomCategory;
         var _loc3_:* = [];
         var _loc9_:int = _container.roomEngine.getRoomObjectCount(_loc1_,_loc2_,RoomObjectCategoryEnum.const_32);
         _loc4_ = 0;
         while(_loc4_ < _loc9_)
         {
            ...
            if(_loc8_ != null)
            {
               _loc5_ = _loc8_.getModel().getNumber(RoomObjectVariableEnum.const_323);
         ...
         _loc3_.sort(compareItems);
         _container.events.dispatchEvent(new RoomWidgetChooserContentEvent(RoomWidgetChooserContentEvent.const_425,_loc3_,_container.sessionDataManager.isAnyRoomController));
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRequestWidgetMessage.const_389:
               handleFurniChooserRequest();
               break;
            case RoomWidgetRequestWidgetMessage.const_309:
               handleUserChooserRequest();
               break;
            case RoomWidgetRoomObjectMessage.const_670:
      ...
      {
         return RoomWidgetEnum.const_238;
      }
   }
}
