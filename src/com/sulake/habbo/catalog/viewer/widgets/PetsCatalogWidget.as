package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_409 >= var_301.length)
         {
            return "";
         }
         var petColor:IPetColor = var_301[var_409] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_570[var_1497] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_409 < 0 || var_409 > var_301.length)
         {
            var_409 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_907.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_186,onTextWindowEvent);
         var_907 = new Map();
         ...
         var_855 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_301 = [];
         var_570 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_855);
               if(_loc11_)
               {
                  var_570 = _loc11_.breeds;
                  var_301 = _loc11_.colors;
                  if(var_301 == null)
                  {
                     var_301 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_570.length)
         {
            _loc12_ = var_570[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_855,_loc12_.id),getRaceLocalizationKey(var_855,_loc12_.id)));
            _loc6_++;
         }
         var_907.add(var_855,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_811,onApproveNameResult);
         _loc5_ = var_907.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_301)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_907.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_409 >= 0 && var_409 < var_301.length)
         {
            _loc7_ = var_301[var_409] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_570[var_1497] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_855,_loc3_.id,_loc2_,AvatarScaleType.const_50);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_322,AvatarAction.const_694);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_38,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_38).clone());
         _loc4_.dispose();
