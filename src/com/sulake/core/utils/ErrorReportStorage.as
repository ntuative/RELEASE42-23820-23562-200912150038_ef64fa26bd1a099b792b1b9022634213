package com.sulake.core.utils
{
   public class ErrorReportStorage
   {
      ...
      private static var var_1087:Map = new Map();
      
      private static var var_743:Map = new Map();
      ...
      public static function getDebugData() : String
      {
         ...
         while(_loc2_ < var_743.length)
         {
            if(_loc2_ == 0)
            {
               _loc1_ = var_743.getWithIndex(_loc2_);
            }
            else
            {
               _loc1_ = _loc1_ + " ** " + var_743.getWithIndex(_loc2_);
            }
            _loc2_++;
         }
         if(_loc1_.length > 450)
         {
            _loc1_ = _loc1_.substr(_loc1_.length - 450);
         }
         return _loc1_;
      }
