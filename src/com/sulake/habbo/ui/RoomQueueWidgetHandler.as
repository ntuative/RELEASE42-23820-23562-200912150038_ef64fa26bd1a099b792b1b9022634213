package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_445:
               ...
               {
                  case RoomSessionQueueEvent.const_808:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_917:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc5_.length > 1)
               {
                  if(_loc4_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_882) != -1)
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_882) + 1;
                     _loc7_ = true;
                  }
                  else
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1282) + 1;
                  }
               }
               else
               {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_683);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_600);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_704);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_683:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_917);
               break;
            case RoomWidgetRoomQueueMessage.const_600:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_808);
               break;
            case RoomWidgetRoomQueueMessage.const_704:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_295,true);
               }
         }
         return null;
      }
