package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         return Math.ceil(this.var_226.length / const_279);
      }
      ...
      {
         if(this.var_797 >= this.getPageCount())
         {
            this.var_797 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      {
         var _loc4_:* = null;
         if(var_226.indexOf(param1) >= 0)
         {
            return;
         }
         var _loc2_:String = param1.name.toLowerCase();
         var _loc3_:int = 0;
         while(_loc3_ < var_226.length)
         {
            _loc4_ = var_226[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               var_226.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         checkPageIndex();
         return Math.min((this.var_797 + 1) * const_279,this.var_226.length);
      }
